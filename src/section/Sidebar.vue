<template>
  ...
  <transition name="sidebar-slide">
    <aside v-show="isOpen || isLgSize" :class="{ '!lg:hidden': isHome }"
      class="fixed inset-0 flex-none h-screen bg-base-100 border-r lg:(static h-auto overflow-y-visible  w-62 block)">
      ...
      <div class="overflow-hidden overflow-y-auto h-full scrolling-touch max-w-2xs h-screen lg:fixed lg:mx-2 lg:w-62">
        <nav class="pt-2 px-1 pl-3 overflow-y-auto text-base lg:(pl-10
 pt-9 text-sm pb-10 h-(screen-18))" aria-label="Docs navigation">
