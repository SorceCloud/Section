...
<template>
  ...
  <transition name="w-modal-bounce" appear>
    <div v-if="isShow" class="w-modal-background" :class="[{ 'w-modal-outside': !outside }]" @click.self="clickOutside">
      <div class="w-modal-box" :class="[`w-modal-${size}`]">
        <button v-show="closeBtn" class="w-modal-btn-close w-btn" @click="dismiss">
